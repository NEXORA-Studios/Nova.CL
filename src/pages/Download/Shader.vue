<script setup lang="ts">
    ...
    async function onSearch() {
        ...
        hits.value = [];
        const data = await modrinthApiAdapter.searchShader(
            searchString.value,
            VersionsFilter.raw,
            ShaderCategoriesFilter.raw,
            ShaderPerformanceImpactFilter.raw,
            ShaderLoadersFilter.raw,
            20,
            ShaderOffsetFilter.offset * 20
        );
        hits.value = data.hits;
        ShaderOffsetFilter.maximum = data.total_hits;
        isLoading.value = false;
    }

    async function onPreviousPage() {
        ShaderOffsetFilter.offset -= 1;
        scrollToTop();
        await onSearch();
    }
    ...
    // ---------- 重整 ----------
    const performanceImpactOrder = ["high", "low", "medium", "potato", "screenshot"];
    ...
    onMounted(() => {
        ...
        onSearch();
        VersionsFilter.init();
        ShaderCategoriesFilter.init();
        ShaderFeaturesFilter.init();
        ShaderPerformanceImpactFilter.init();
        ShaderLoadersFilter.init();
    });
</script>

<template>
    <div class="w-full h-full p-6 overflow-hidden">
        <SearchInput v-model="searchString" :placeholder="t('Main.Download/Shader.GlobalSearchPlaceholder')" @click="onSearch" />
        ...
        <div
            ...
            <section class="flex flex-col gap-2">
                ...
                <ModrinthSearchFilter
                    ...
                    :items="sortByOrderField(ShaderPerformanceImpactFilter.filtered.value, performanceImpactOrder, 'name')" />
                ...
                <!-- Page -->
                <ModrinthSearchPage
                    :offset="ShaderOffsetFilter.offset"
                    :maximum="ShaderOffsetFilter.maximum"
