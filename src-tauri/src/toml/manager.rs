...
use crate::toml::error::{ConfigError, GlobalConfigError, ProfileConfigError, InstanceConfigError};
use crate::toml::types::{GlobalConfig, ProfileConfig, InstanceConfig};
...
impl ConfigManager {
    ...
    /// 加载实例配置
    pub fn load_instance_config(&self, instance_id: &str) -> Result<InstanceConfig, InstanceConfigError> {
        let config_path = self.instance_config_path(instance_id);
        
        if !config_path.exists() {
            return Err(InstanceConfigError::InstanceNotFound(instance_id.to_string()));
        }
    ...
    /// 保存实例配置
    pub fn save_instance_config(&self, instance_id: &str, config: &InstanceConfig) -> Result<(), InstanceConfigError> {
...
/// 保存实例配置
pub fn save_instance_config(instance_id: &str, config: &InstanceConfig) -> Result<(), InstanceConfigError> {
